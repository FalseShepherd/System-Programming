...
void BitMap::ThreadWorker(SharedQueue<Range> *workQueue, BitMap *result)
{
	Range work;
	while (true)
	{
		// read work from queue
		while (workQueue->Remove(work) == false)
		{
		...
		this->DetectEdge(work.from, work.to, result);
	}
}
...
BitMap* BitMap::DetectEdges(int threadCount) const
{
	...
	{
		threads[thread] = new std::thread(&BitMap::ThreadWorker, *this, &workQueue, edge);
	}
	for (uint64_t x = 0; x < this->height-1; x ++)
	{
		workQueue.Add({ x*this->width, (x+1)*this->width});
	}
